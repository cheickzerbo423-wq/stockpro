...
// renommer, suspendre/réactiver et supprimer des espaces "entreprise"
// (= les sociétés clientes de WariGest), et avoir une vue d'ensemble de
// l'activité de chacune. Toutes les routes de ce contrôleur sont protégées
// par le middleware `superAdminOnly` (categorie = 'SuperAdmin').
...
async function getAll(req, res) {
  try {
    const result = await db.query(`
      SELECT
        e.id, e.nom, e.slug, e.actif, e.created_at, e.updated_at,
...
async function create(req, res) {
  ...
  try {
    const { nom, admin_login, admin_mdp } = req.body;
    if (!nom || !nom.trim())
    ...
    // Le login reste GLOBALEMENT unique sur la plateforme (même règle que dans
    // utilisateursController.create — voir commentaire associé).
    ...
    // Garantir l'unicité du slug (contrainte UNIQUE sur entreprises.slug)
    ...
    const ent = await client.query(
      `INSERT INTO entreprises (nom, slug, actif) VALUES ($1, $2, TRUE) RETURNING *`,
      [nomTrim, slug]
    );
...
// Suspendre coupe immédiatement l'accès de tous les utilisateurs de cette
// entreprise (vérifié dans le middleware authenticate et au login).
...
// d'une entreprise et de TOUTES ses données (articles, ventes, achats,
// clients, factures, utilisateurs…). Aucune contrainte ON DELETE CASCADE
// n'existe sur entreprise_id (choix prudent fait lors de la migration —
// voir notes sur les contraintes DB), donc on supprime manuellement, dans
// le bon ordre vis-à-vis des clés étrangères, au sein d'une transaction.
...
module.exports = { getAll, create, update, toggleStatut, remove };
