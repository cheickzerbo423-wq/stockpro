    ...
    const [totaux, alertes, caAnnee, topClients, recentFactures, creancesClients, caParArticle] = await Promise.all([
      db.query(`
        SELECT
          ...
          -- cogs = coût des marchandises réellement vendues (prix_achat figé à la
          -- vente × qté vendue). prix_achat est capturé sur lignes_vente au moment
          -- de la vente : insensible aux changements ultérieurs du prix d'achat.
          (SELECT COALESCE(SUM(lv.quantite * lv.prix_achat), 0)
