// src/pages/Dashboard.jsx — Tableau de bord Modern SaaS
import {
  BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer,
  LineChart, Line, CartesianGrid, Area, AreaChart,
} from "recharts";
import { useDashboard } from "../hooks/useApi";
import { fmt, fmtN, Spinner, ErrorBox, SectionTitle } from "../components/UI";

const MOIS_LABELS = ["Jan","Fév","Mar","Avr","Mai","Jun","Jul","Aoû","Sep","Oct","Nov","Déc"];

/* ── Tooltip graphique ── */
const GraphTooltip = ({ active, payload, label }) => {
  if (!active || !payload?.length) return null;
  return (
    <div className="bg-gray-900 text-white rounded-xl px-3 py-2.5 text-xs shadow-xl border border-white/10">
      <div className="font-bold text-gray-400 mb-1">{label}</div>
      {payload.map((p) => (
        <div key={p.dataKey} className="flex items-center gap-2">
          <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: p.color }} />
          <span>{p.name} :</span>
          <strong>{fmt(p.value)}</strong>
        </div>
      ))}
    </div>
  );
};

/* ── KPI Card ── */
function KpiCard({ icon, label, value, sub, color, trend }) {
  const palette = {
    blue:   { ring: "#B3BFFF", bg: "#E6EAFF", text: "#0023FF" },
    green:  { ring: "#A7F3D0", bg: "#ECFDF5", text: "#059669" },
    red:    { ring: "#FECACA", bg: "#FEF2F2", text: "#DC2626" },
    amber:  { ring: "#FDE68A", bg: "#FFFBEB", text: "#D97706" },
    purple: { ring: "#DDD6FE", bg: "#F5F3FF", text: "#7C3AED" },
  };
  const c = palette[color] || palette.blue;
  return (
    <div className="group bg-white rounded-2xl p-4 sm:p-5 border border-gray-100 cursor-default transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg"
      style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
      <div className="flex items-start justify-between mb-3 sm:mb-4">
        <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-2xl flex items-center justify-center flex-shrink-0 transition-transform duration-200 group-hover:scale-105"
          style={{ backgroundColor: c.bg, border: `1.5px solid ${c.ring}` }}>
          <span className="text-lg sm:text-xl">{icon}</span>
        </div>
        {trend !== undefined && (
          <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-lg ${
            trend >= 0 ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-500"
          }`}>
            <span>{trend >= 0 ? "▲" : "▼"}</span>
            {Math.abs(trend)}%
          </span>
        )}
      </div>
      <div className="text-lg sm:text-[22px] font-black text-gray-900 leading-tight tracking-tight break-words">{value}</div>
      <div className="text-xs font-semibold text-gray-400 mt-1 leading-tight">{label}</div>
      {sub && (
        <div className="mt-3 pt-3 border-t border-gray-50 text-[10px] text-gray-400 font-medium break-words">{sub}</div>
      )}
    </div>
  );
}

/* ── Barre de progression colorée ── */
function ProgressBar({ value, max, color }) {
  const pct = max > 0 ? Math.min(100, Math.round((value / max) * 100)) : 0;
  return (
    <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden mt-1">
      <div className="h-full rounded-full transition-all duration-700"
        style={{ width: `${pct}%`, backgroundColor: color }} />
    </div>
  );
}

/* ── Composant principal ── */
export default function Dashboard() {
  const { data, loading, error, reload } = useDashboard();
  const annee = new Date().getFullYear();

  if (loading) return <Spinner label="Chargement du tableau de bord…" />;
  if (error)   return <ErrorBox message={error} onRetry={reload} />;
  if (!data)   return null;

  const { kpis, alertes_stock = [], ca_par_mois = [], top_clients = [], recent_factures = [] } = data;

  const tauxRec = kpis.ca_facture > 0 ? Math.round((kpis.encaisse / kpis.ca_facture) * 100) : 0;

  const graphData = MOIS_LABELS.map((label, i) => {
    const moisKey = `${annee}-${String(i + 1).padStart(2, "0")}`;
    const curr = ca_par_mois.find((r) => r.mois === moisKey);
    return { label, ca: curr ? parseInt(curr.ca) : 0 };
  });
  const hasAnyData = graphData.some((d) => d.ca > 0);

  const beneficeColor = kpis.benefice >= 0 ? "#059669" : "#DC2626";

  return (
    <div className="space-y-6 max-w-7xl mx-auto">

      {/* ── En-tête ── */}
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-gray-900 leading-tight">Tableau de Bord</h2>
          <p className="text-sm text-gray-400 mt-0.5 font-medium">
            {new Date().toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
          </p>
        </div>
        <button onClick={reload}
          className="flex items-center gap-2 text-xs font-bold text-gray-500 hover:text-[#0023FF] bg-white border border-gray-200 hover:border-[#B3BFFF] px-3.5 py-2 rounded-xl shadow-sm transition-all duration-150 hover:-translate-y-0.5">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5">
            <polyline points="23 4 23 10 17 10"/>
            <path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/>
          </svg>
          Actualiser
        </button>
      </div>

      {/* ── KPIs ── */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <KpiCard
          icon="📈"
          label={`Chiffre d'Affaires ${annee}`}
          value={fmt(kpis.ca_total)}
          sub={`Dépenses : ${fmt(kpis.depenses_total)}`}
          color="blue"
        />
        <KpiCard
          icon="💰"
          label="Bénéfice Net"
          value={fmt(kpis.benefice)}
          sub={`Marge : ${kpis.ca_total > 0 ? Math.round((kpis.benefice / kpis.ca_total) * 100) : 0}%`}
          color={kpis.benefice >= 0 ? "green" : "red"}
        />
        <KpiCard
          icon="📦"
          label="Valeur du Stock"
          value={fmt(kpis.valeur_stock || 0)}
          sub={`${fmtN(kpis.nb_articles)} articles actifs`}
          color="purple"
        />
        <KpiCard
          icon="🧾"
          label="Factures Émises"
          value={fmtN(kpis.nb_factures)}
          sub={`${kpis.factures_impayees} impayée(s) · ${fmt(kpis.montant_a_recouvrer)}`}
          color={parseInt(kpis.factures_impayees) > 0 ? "amber" : "green"}
        />
      </div>

      {/* ── Taux de recouvrement ── */}
      <div className="bg-white rounded-2xl border border-gray-100 px-6 py-5"
        style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
        <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-2.5">
            <div className="w-1 h-4 rounded-full" style={{ backgroundColor: "#0023FF" }} />
            <h3 className="text-xs font-bold text-gray-500 uppercase tracking-[0.08em]">Taux de Recouvrement</h3>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-gray-400 font-medium">Encaissé : <strong className="text-gray-700">{fmt(kpis.encaisse)}</strong></span>
            <span className="text-xs text-gray-400 font-medium">Créances : <strong className="text-red-500">{fmt(kpis.montant_a_recouvrer)}</strong></span>
            <span className={`text-lg font-black ${tauxRec >= 80 ? "text-emerald-600" : tauxRec >= 50 ? "text-amber-500" : "text-red-500"}`}>
              {tauxRec}%
            </span>
          </div>
        </div>
        <div className="h-3 bg-gray-100 rounded-full overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-1000 ${tauxRec >= 80 ? "bg-emerald-500" : tauxRec >= 50 ? "bg-amber-400" : "bg-red-400"}`}
            style={{ width: `${tauxRec}%` }}
          />
        </div>
        <div className="flex justify-between mt-1.5 text-[10px] text-gray-300 font-medium">
          <span>0%</span><span>50%</span><span>100%</span>
        </div>
      </div>

      {/* ── Graphique évolution CA ── */}
      <div className="bg-white rounded-2xl border border-gray-100 p-6"
        style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
        <SectionTitle>Évolution du CA — {annee}</SectionTitle>
        {!hasAnyData ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3">
            <div className="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center text-2xl">📊</div>
            <p className="text-sm text-gray-400 font-medium">Aucune vente enregistrée cette année</p>
          </div>
        ) : (
          <ResponsiveContainer width="100%" height={200}>
            <AreaChart data={graphData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              <defs>
                <linearGradient id="caGrad" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#0023FF" stopOpacity={0.15}/>
                  <stop offset="100%" stopColor="#0023FF" stopOpacity={0}/>
                </linearGradient>
              </defs>
              <CartesianGrid strokeDasharray="3 3" stroke="#F1F5F9" vertical={false} />
              <XAxis dataKey="label" tick={{ fontSize: 10, fill: "#94a3b8", fontWeight: 600 }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 10, fill: "#94a3b8", fontWeight: 600 }} axisLine={false} tickLine={false}
                tickFormatter={(v) => v >= 1e6 ? (v/1e6).toFixed(1)+"M" : v >= 1000 ? (v/1000).toFixed(0)+"k" : v} />
              <Tooltip content={<GraphTooltip />} />
              <Area type="monotone" dataKey="ca" name="CA" stroke="#0023FF" strokeWidth={2.5}
                fill="url(#caGrad)" dot={false} activeDot={{ r: 5, fill: "#0023FF", stroke: "#fff", strokeWidth: 2 }} />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </div>

      {/* ── 3 colonnes bas ── */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">

        {/* Top clients */}
        <div className="bg-white rounded-2xl border border-gray-100 p-5"
          style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          <SectionTitle>Top Clients</SectionTitle>
          {top_clients.length === 0 ? (
            <div className="flex flex-col items-center py-8 gap-2 text-center">
              <span className="text-2xl">👥</span>
              <p className="text-xs text-gray-400 font-medium">Aucune vente cette année</p>
            </div>
          ) : (
            <div className="space-y-3">
              {top_clients.map((c, i) => {
                const maxCa = top_clients[0].ca;
                const pct = Math.round((c.ca / maxCa) * 100);
                return (
                  <div key={c.client_nom}>
                    <div className="flex items-center justify-between text-xs mb-1">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className="w-5 h-5 rounded-lg bg-gray-100 flex items-center justify-center text-[10px] font-black text-gray-400 flex-shrink-0">
                          {i + 1}
                        </span>
                        <span className="font-semibold text-gray-700 truncate">{c.client_nom}</span>
                      </div>
                      <span className="font-bold text-gray-900 ml-2 flex-shrink-0">{fmt(c.ca)}</span>
                    </div>
                    <ProgressBar value={c.ca} max={maxCa} color="#0023FF" />
                  </div>
                );
              })}
            </div>
          )}
        </div>

        {/* Dernières factures */}
        <div className="bg-white rounded-2xl border border-gray-100 p-5"
          style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          <SectionTitle>Dernières Factures</SectionTitle>
          <div className="space-y-2.5">
            {recent_factures.length === 0 && (
              <div className="flex flex-col items-center py-8 gap-2 text-center">
                <span className="text-2xl">🧾</span>
                <p className="text-xs text-gray-400 font-medium">Aucune facture</p>
              </div>
            )}
            {recent_factures.map((f) => {
              const paid = f.statut || parseFloat(f.reste || 0) <= 0;
              return (
                <div key={f.code} className="flex items-center justify-between gap-2 p-2.5 rounded-xl hover:bg-gray-50 transition-colors">
                  <div className="min-w-0">
                    <div className="text-xs font-black text-[#0023FF] font-mono tracking-tight">{f.code}</div>
                    <div className="text-[10px] text-gray-400 font-medium truncate mt-0.5">{f.client_nom}</div>
                  </div>
                  <div className="text-right flex-shrink-0 flex flex-col items-end gap-1">
                    <div className="text-xs font-bold text-gray-800">{fmt(f.montant)}</div>
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${paid ? "bg-emerald-50 text-emerald-600" : "bg-red-50 text-red-500"}`}>
                      {paid ? "✓ Réglée" : "⏳ Impayée"}
                    </span>
                  </div>
                </div>
              );
            })}
          </div>
        </div>

        {/* Alertes stock */}
        <div className="bg-white rounded-2xl border border-gray-100 p-5"
          style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          <SectionTitle
            action={alertes_stock.length > 0 && (
              <span className="text-[10px] font-black bg-red-500 text-white px-2 py-0.5 rounded-full">
                {alertes_stock.length}
              </span>
            )}
          >
            Alertes Stock
          </SectionTitle>

          {alertes_stock.length === 0 ? (
            <div className="flex items-center gap-3 bg-emerald-50 border border-emerald-100 rounded-xl px-3.5 py-3">
              <div className="w-8 h-8 bg-emerald-100 rounded-xl flex items-center justify-center flex-shrink-0">
                <svg viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2.5" className="w-4 h-4">
                  <polyline points="20 6 9 17 4 12"/>
                </svg>
              </div>
              <div>
                <div className="text-xs font-bold text-emerald-700">Stocks en ordre</div>
                <div className="text-[10px] text-emerald-500">Aucune rupture détectée</div>
              </div>
            </div>
          ) : (
            <div className="space-y-2">
              {alertes_stock.map((a) => (
                <div key={a.code}
                  className="flex items-center justify-between px-3 py-2.5 rounded-xl border transition-colors hover:bg-gray-50"
                  style={{ borderColor: a.stock_restant <= 0 ? "#FECACA" : "#FDE68A", background: a.stock_restant <= 0 ? "#FEF2F2" : "#FFFBEB" }}>
                  <div className="min-w-0 flex-1">
                    <div className="text-xs font-bold text-gray-800 truncate">{a.libelle}</div>
                    <div className="text-[10px] text-gray-400 font-mono">{a.code}</div>
                  </div>
                  <div className="text-right shrink-0 ml-2">
                    <div className={`text-sm font-black ${a.stock_restant <= 0 ? "text-red-600" : "text-amber-600"}`}>
                      {a.stock_restant <= 0 ? "Rupture" : `${a.stock_restant} u.`}
                    </div>
                    <div className="text-[9px] text-gray-400">min. {a.stock_min}</div>
                  </div>
                </div>
              ))}
            </div>
          )}
        </div>
      </div>

      {/* ── Bar chart top clients ── */}
      {top_clients.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 p-6"
          style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.06)" }}>
          <SectionTitle>CA par Client — {annee}</SectionTitle>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={top_clients} barSize={32} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              <XAxis dataKey="client_nom" tick={{ fontSize: 10, fill: "#94a3b8", fontWeight: 600 }} axisLine={false} tickLine={false}
                tickFormatter={(v) => v.split(" ")[0].slice(0, 10)} />
              <YAxis tick={{ fontSize: 10, fill: "#94a3b8", fontWeight: 600 }} axisLine={false} tickLine={false}
                tickFormatter={(v) => v >= 1e6 ? (v/1e6).toFixed(1)+"M" : v >= 1000 ? (v/1000).toFixed(0)+"k" : v} />
              <Tooltip content={<GraphTooltip />} />
              <Bar dataKey="ca" name="CA" fill="#0023FF" radius={[8, 8, 0, 0]}
                activeBar={{ fill: "#0019CC", stroke: "#0023FF", strokeWidth: 1.5 }} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}

      {/* ── Résumé bénéfice ── */}
      <div className="rounded-2xl border p-6 flex flex-wrap items-center justify-between gap-4"
        style={{
          background: kpis.benefice >= 0 ? "#ECFDF5" : "#FEF2F2",
          borderColor: kpis.benefice >= 0 ? "#A7F3D0" : "#FECACA",
          boxShadow: "0 1px 3px rgba(0,0,0,0.06)"
        }}>
        <div>
          <div className="text-xs font-bold uppercase tracking-widest mb-1"
            style={{ color: beneficeColor + "99" }}>
            Bénéfice net — {annee}
          </div>
          <div className="text-3xl font-black" style={{ color: beneficeColor }}>
            {fmt(kpis.benefice)}
          </div>
          <div className="text-xs mt-1.5" style={{ color: beneficeColor + "80" }}>
            CA {fmt(kpis.ca_total)} — Dépenses {fmt(kpis.depenses_total)}
          </div>
        </div>
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl flex-shrink-0"
          style={{ background: beneficeColor + "15" }}>
          {kpis.benefice >= 0 ? "🚀" : "📉"}
        </div>
      </div>

    </div>
  );
}
